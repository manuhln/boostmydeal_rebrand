...
class TTSConfig(BaseModel):
    ...
    provider_name: str = Field(
        ..., description="TTS provider: 'eleven_labs', 'openai', or 'smallest_ai'"
    )
    voice_id: str = Field(..., description="Voice ID for the TTS provider")
    model_id: str = Field(
        default="",
        description="Model ID for TTS (optional, e.g., 'waves-v2' for Smallest.ai)",
    )
...
class STTConfig(BaseModel):
    ...
    model: str = Field(
        default="nova-2",
        description="STT model to use (e.g., 'nova-2' for Deepgram, 'gpt-4o-mini-transcribe' for OpenAI)",
    )
...
class CallConfig(BaseModel):
    ...
    user_speak_first: bool = Field(
        default=True, description="If true, user speaks first"
    )
    agent_prompt_preamble: str = Field(..., description="System prompt for the agent")
    agent_generate_responses: bool = Field(
        default=True, description="Enable agent responses"
    )
    ...
    agent_speed: float = Field(
        default=1.0, ge=0.5, le=2.0, description="Agent speaking speed"
    )
    webhook_url: Optional[str] = Field(
        None, description="URL to send webhook events (optional)"
    )
    use_knowledge_base: bool = Field(
        default=True, description="Enable Pinecone knowledge base for RAG"
    )
    ...
    recording: bool = Field(
        default=False, description="Enable call recording with GCS upload"
    )
    recording_expiration_days: int = Field(
        default=30, ge=1, le=365, description="Recording URL expiration in days"
    )
    previous_call_summary: Optional[str] = Field(
        None,
        description="Summary of previous call history with this customer for conversation continuity",
    )
    current_date: Optional[str] = Field(
        None, description="Current date (e.g., 'Monday, November 3, 2025')"
    )
    current_time: Optional[str] = Field(
        None, description="Current time (e.g., '02:36:37 PM')"
    )
    enable_call_transfer: bool = Field(
        default=False, description="Enable call transfer to human agent"
    )
    transfer_phone_number: Optional[str] = Field(
        None,
        description="Phone number to transfer call to (E.164 format, e.g., '+1234567890')",
    )
    user_tags: list[str] = Field(
        default_factory=list,
        description="User-defined tags to detect in conversation (e.g., ['follow up', 'schedule meet'])",
    )
    system_tags: list[str] = Field(
        default_factory=list,
        description="System-defined tags to detect in conversation (e.g., ['interested if call more than 1 minute'])",
    )
    livekit_sip_trunk_id: str = Field(
        ..., description="LiveKit SIP trunk ID for making outbound calls"
    )
    keyboard_sound: bool = Field(
        default=False,
        description="Enable keyboard typing sound effects when agent notes user data",
    )

    mode: str = Field(
        default="pipeline", description="Execution mode: 'pipeline' or 'realtime'"
    )
    ...
    response_guideline: Optional[str] = Field(None, description="Response guidelines")
    fallback: Optional[str] = Field(None, description="Fallback responses")
    remember_lead_preference: bool = Field(
        default=False, description="Remember lead preferences"
    )
